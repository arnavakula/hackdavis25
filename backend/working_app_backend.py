...
client = Cerebras(
    api_key="csk-4wkehm3t2vm8c64kkwhh3y6jenfhvetptnppxpv9xpmrtxcf"
)
...
# image_paths = glob.glob(os.path.join("processed_frames", "*.jpg"))

# # 2) Fire the chat API with a single message containing all of them
# res = ollama.chat(
#     model="llava",
#     messages=[
#         {
#             "role":    "user",
#             "content": "I am giving you the images of endoscopy images frames,What is in these images? ",
#             "images":  image_paths
#         }
#     ]
# )
...
def receive_frames():
    global frames
    data = request.get_json()
    # handle both direct list POSTs and { "frames": [...] } bodies
...
def next_steps():
    ...
    # normalize if someone accidentally set frames to one big string
    if isinstance(frames, str):
        frames = [line.strip() for line in frames.splitlines() if line.strip()]

    # 1) Decode, resize, compress, save locally, re-encode
    for idx, uri in enumerate(frames):
        ...
        # save to disk
        file_path = os.path.join(OUTPUT_DIR, f"frame_{idx}.jpg")
...
# 2) Fire the chat API with a single message containing all of them
    res = ollama.chat(
        ...
        messages=[
            {
                "role":    "user",
                "content": "I am giving you the images of endoscopy images frames,What is in these images give me in detail ? ",
                "images":  image_paths
            }
        ]
    )
    ...
    # 4) Construct your LLM user message
    user_message = f"""
use the description of the images you are seeing below:
...
Provide a short answer, no more than 30 words.
"""
...
use the description of the images you are seeing below:
  ...
  what patterns doo you notice? and suggest some measure to take here.
    Provide a short answer, no more than 30 words.
    """
    messages = [
